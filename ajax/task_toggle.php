<?php
/**
 * Toggle completamento task (AJAX JSON).
 */
...
$chk = db()->prepare('SELECT is_completed FROM tasks WHERE id = ? AND user_id = ?');
...
json_response(['success' => true, 'message' => 'Stato aggiornato', 'is_completed' => (int) $row['is_completed']]);
